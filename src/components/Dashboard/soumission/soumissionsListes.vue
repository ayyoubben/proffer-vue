...
export default {
    ...
    methods: {
        handleGetLots() {
            getSoumissionsByLot(this.$store.state.newLot._id, localStorage.getItem("adminToken")).then(res => {this.soums = res.data 
            console.log(res.data)})
        },
        goToSoumission(soum) {
            this.$store.commit('setNewSoum', soum)                
            this.$router.push("/dashboard/soumissions/soumission")
        }
    },
}
</script>
