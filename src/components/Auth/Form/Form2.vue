<template>
    <div>
        ...
        <div class="col-lg-7 mx-0 px-0">
            <div class="progress">
               <div aria-valuemax="100" aria-valuemin="0" aria-valuenow="50" class="progress-bar gradient-custom-4" role="progressbar" style="width: 33%"></div>
            </div>
            <div id="qbox-container">
               <form class="needs-validation" id="form-wrapper" method="post" name="form-wrapper" novalidate="">
                 
                        <h4>Votre affiliation sociale:</h4>
                        <div class="row mt-5">
                           <div class="col-lg-7 ">
                              <div class="mt-1 form-floating">   
                                 <input v-model="casnos" class="form-control form-control-sm" id="MatriculeCasnos" name="MatriculeCasnos" type="number" placeholder="Matricule d'affiliation CASNOS" required="true"/>
                                 <label for="MatriculeCasnos">Matricule d'affiliation CASNOS</label> 
                              </div>
                           </div>
                           <div class="col-lg-5 col-md-12 col-sm-12 col-12">
                                 <input @change="handleCasnosFile()" ref="casnos" class="form-control mt-1" accept=".pdf" type="file" id="formFile" required="true"/>
                           </div>
                        </div>
                        <div class="row mt-5">
                           <div class="col-lg-7 ">
                              <div class="mt-1 form-floating">   
                                 <input v-model="cacopath" class="form-control form-control-sm" id="Cacobatph" name="Cacobatph" type="number" placeholder="Matricule Cacobatph" required="true"/>
                                 <label for="Cacobatph">Matricule Cacobatph</label> 
                              </div>
                           </div>
                           <div class="col-lg-5 col-md-12 col-sm-12 col-12">
                                 <input @change="handleCacopathFile()" ref="cacopath" class="form-control mt-1" type="file" id="CacobatphFile" required="true"/>
                           </div>
                        </div>
...
import {createInscEnr, updateInscEnr} from '../../../utils/inscription/inscription'
    ...
    export default {  
        ...
        created () {
            this.casnos = localStorage.getItem("inscCasnos")
            this.cacopath = localStorage.getItem("inscCacopath")
        },      
        methods:{
            handlePrevBtn() {
                if( this.casnos != undefined ) {
                    localStorage.setItem("inscCasnos", this.casnos)
                        
                }
                if( this.cacopath != undefined ) {
                    localStorage.setItem("inscCacopath", this.cacopath)
                }
            ...
            handleNextBtn() {
                ...
                this.$router.push('/signup/form3')

            },
            handleCasnosFile() {
                this.casnosFile = this.$refs.casnos.files[0]
                console.log(this.casnosFile)
                this.casnosFile
                const vm = this
                if(this.casnosFile){
                    ...
                    reader.readAsDataURL(this.casnosFile);
                }
                
            },
            handleCacopathFile() {
                this.cacopathFile = this.$refs.cacopath.files[0]
                console.log(this.cacopathFile)
                this.cacopathFile
                const vm = this
                if(this.cacopathFile){
                    const reader = new FileReader();
                    reader.onload = function(){
                        const result = reader.result;
                        const file = {}
                        //console.log(result)
                        file.name = "cacopath file"
                        file.src = result
                    ...
                    this.$store.commit("setInscDocuments", this.$store.state.inscDocuments)
                    reader.readAsDataURL(this.cacopathFile);
                }
